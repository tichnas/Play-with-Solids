...
GLFWwindow *initGLFW(int width, int height) {
  ...
  window = glfwCreateWindow(width, height, "Sample OpenGL 3.3 Application",
                            NULL, NULL);
  ...
  GLenum err = glewInit();
  if (err != GLEW_OK) {
    cout << "Error: Failed to initialise GLEW : " << glewGetErrorString(err)
         << endl;
    exit(1);
  }
  glfwSwapInterval(1);
  ...
  glfwSetCharCallback(window,
                      keyboardChar);  // simpler specific character handling
...
struct VAO *create3DObject(GLenum primitive_mode, int numVertices,
  ...
  glVertexAttribPointer(0,         // attribute 0. Vertices
                        3,         // size (x,y,z)
                        GL_FLOAT,  // type
                        GL_FALSE,  // normalized?
                        0,         // stride
  ...
  glBufferData(GL_ARRAY_BUFFER, 3 * numVertices * sizeof(GLfloat),
               color_buffer_data, GL_STATIC_DRAW);  // Copy the vertex colors
...
void draw3DObject(struct VAO *vao) {
  ...
  // Draw the geometry !
  glDrawArrays(vao->PrimitiveMode, 0,
               vao->NumVertices);  // Starting from vertex 0; 3 vertices total
                                   // -> 1 triangle
}
